...
def compute_ring_skymaps(ring_i: int = 0, E_gamma: float = 10, nside: int = 128):
    ...
    for name, model in gas_models.items():
        fitsname = f'!fits/Pi0-{name}-ring{ring}-{nside}-{E_gamma}GeV-gamma-skymap.fits.gz'
...
if __name__ == "__main__":
    ...
    plot_map_mollview(f'fits/Pi0-HI-ring{ring}-{nside}-{E_gamma}GeV-gamma-skymap.fits.gz', 
                      f'Gamma-Pi0-HI-mollview-ring{ring}-{nside}-{E_gamma}GeV', 
                      min_map=-4.0, max_map=-1.0)
    plot_map_mollview(f'fits/Pi0-H2-ring{ring}-{nside}-{E_gamma}GeV-gamma-skymap.fits.gz', 
                      f'Gamma-Pi0-H2-mollview-ring{ring}-{nside}-{E_gamma}GeV', 
